# SKEDTool_JP — JVN / VERA Schedule Tool
...
VERA画面で他局を含むVEXを読み込むと、Vm・Vr・Vo・Vsのみを残し、除外した局を通知します。
4局の定義やスキャンへの参加が不足する場合はエラーを表示します。
...
取得後の計算・描画中も待機表示を続けます。処理中の連打は無視し、Plot欄は再利用します。
Dockerビルド時にはMatplotlibのフォントキャッシュも作成し、初回の探索待ちを減らします。
...
具体的な設定値と操作手順は [Renderへのデプロイ手順](docs/render.md) を参照してください。
